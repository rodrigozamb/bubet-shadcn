...
export default function AdminCreateEventPage({}) {
  ...
  const handleCreate = async () => {
    ...
    formData.append("local",location)
    
    formData.append("date", ds[2]+"-"+ds[1]+"-"+ds[0]+" 00:00:00.000")
    formData.append("starts_at",ds[2]+"-"+ds[1]+"-"+ds[0]+" "+sa[0]+":"+sa[1]+":00.000")
    formData.append("ends_at",ds[2]+"-"+ds[1]+"-"+ds[0]+" "+ea[0]+":"+ea[1]+":00.000")

    formData.append("banner",avatar!)
    

    const req_competitors = selectedCompetitors.map((cmp) => cmp.id)
    const req_judges = selectedJudges.map((jdg) => jdg.id)
    const req_banner_types = selectedEstandartes.map((ests) => ests.id)
  ...
  useEffect(() => {
    ...
    api.get(`/events/${event_id}`, { withCredentials: true })
        .then((enventRequestResult) => {
            setName(enventRequestResult.data.name)
            setDescripion(enventRequestResult.data.description)
            setDate(enventRequestResult.data.date.split("T")[0])
            setStartsAt(enventRequestResult.data.starts_at.split("T")[1].split(".")[0])
            setEndsAt(enventRequestResult.data.ends_at.split("T")[1].split(".")[0])
            setLocation(enventRequestResult.data.local)

            api.get(`/judges`, {withCredentials: true})
                .then((allJudgesResponse) =>{

                    setJudges(allJudgesResponse.data)
                    setSelectedJudges(enventRequestResult.data.judges)

                    api.get(`/competitors`, {withCredentials: true})
                        .then((allCompetitorsResponse) => {
                            
                            setCompetitors(allCompetitorsResponse.data)
                            setSelectedCompetitors(enventRequestResult.data.competitors)


                            api.get(`/estandartes`, {withCredentials: true})
                            .then((allEstandartesResult) => {

                                setEstandartes(allEstandartesResult.data)
                                setSelectedEstandartes(enventRequestResult.data.event_banner_types)
                            })
                        })
                })
        })
      ...
      <>
        <div className="flex flex-col justify-center items-center justify-begin bg-blue-900 w-screen h-300">
            ...
            <div className="flex flex-col items-center bg-amber-100 w-350 py-5">
                <span className="font-medium text-2xl my-4">Edite um evento</span>
